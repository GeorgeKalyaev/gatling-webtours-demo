...
### 3.1. Содержимое `_full`, выгрузка и папки на ПК (шаги 7–16)

7. Внутри каталога вида **`debug-…_full`** (после распаковки zip или на генераторе) лежат две папки **`without_groups`** и **`with_groups`**; в каждой из них — свой **`simulation.log`** (с группами и без соответственно).

8. То же на **втором** генераторе. В сумме у вас **четыре** файла лога с двух машин: по одному в **`with_groups`** и **`without_groups`** на каждом хосте.

9. **Скопируйте / выгрузите** все четыре лога на локальный ПК (SFTP, WinSCP и т.д.).

10. Создайте рабочую папку, например **`task2`**, и положите в неё скрипт **[`MergeSimulation.py`](../../tools/reporting/MergeSimulation.py)**.

11. Внутри **`task2`** создайте папку, например **`test1`**, и **ещё одну вложенную** с тем же именем **`test1`**, чтобы путь был **`Отчет1\task2\test1\test1`** (корень `Отчет1` — по желанию, как в вашей структуре).

12. Во **внутренней** папке **`test1`** создайте две папки: **`with_groups`** и **`without_groups`**.

13. В **`with_groups`** поместите **`simulation.log`** с **первого** генератора (из его `…_full/with_groups/`) и **`simulation.log`** со **второго** (из его `…_full/with_groups/`). Так как имена совпадают, **один из файлов переименуйте** (в примере ниже — `simulation1.log`).

14. В **`without_groups`** — то же для логов из **`without_groups`** на обоих генераторах; второй файл при необходимости назовите, например, **`simulation2.log`** (или иная схема имён, лишь бы в одной папке не было двух одинаковых имён).

15. В результате **`with_groups`** должна выглядеть так: два текстовых лога рядом (**`simulation.log`** и **`simulation1.log`** в примере).

   ![Проводник: Отчет1\task2\test1\test1\with_groups — два лога с двух генераторов](../images/two-generators-with-groups-two-logs.png)

16. Папка **`without_groups`** должна выглядеть аналогично: два лога (**`simulation.log`** и **`simulation2.log`** в примере) — по одному с каждого генератора.

   ![Проводник: Отчет1\task2\test1\test1\without_groups — два лога без групп](../images/two-generators-without-groups-two-logs.png)

Краткая схема:

```text
Отчет1/task2/MergeSimulation.py
Отчет1/task2/test1/test1/with_groups/     simulation.log + simulation1.log
...
17. В [`MergeSimulation.py`](../../tools/reporting/MergeSimulation.py) задать **`input_folder`** на каталог с двумя логами для первого слияния — **`…/test1/test1/with_groups`**. Путь указывайте **с прямыми слэшами** `/` (в том числе на Windows), например:
   ...
   Подставьте свой каталог пользователя и диск; конец пути — **`with_groups`**, не файл.

18. В **CMD** перейти в папку **`task2`** (где лежит скрипт) и выполнить: `python MergeSimulation.py`. В выводе — обработка каждого **`*.log`** в `input_folder` и строка о сохранении объединённого файла. Имя файла задаётся переменной **`output_file`** в скрипте (в репозитории по умолчанию **`merged_simulation.log`**; в учебном примере на скриншоте — **`общий_лог.log`**, если так задано у вас).

   ![CMD: MergeSimulation.py склеивает simulation.log и simulation1.log из with_groups](../images/MergeSimulation-cmd-with-groups.png)

19. **Итог после первого запуска:** объединённый лог **с группами** появляется **на одном уровне со скриптом** — в папке **`task2`** рядом с **`MergeSimulation.py`** (если для **`output_file`** не задан полный путь). На скриншоте — **`общий_лог.log`**; у вас имя совпадает с **`output_file`** в скрипте (в репозитории по умолчанию **`merged_simulation.log`**).

   ![Проводник: Отчет1\task2 — скрипт и объединённый лог](../images/MergeSimulation-merged-log-in-task2.png)

20. **Для `with_groups`:** перенесите объединённый файл из **`task2`** (в примере — **`общий_лог.log`**, у вас — как в **`output_file`**) в **`Отчет1\task2\test1\test1\with_groups\`** (каталог называется **`with_groups`**, с буквой **s** на конце). На скриншоте — промежуточное состояние: в папке лежат исходные **`simulation.log`** / **`simulation1.log`** и уже скопированный **`общий_лог.log`** (размер ~сумма двух исходных). Далее **удалите** или уберите в сторону **`simulation.log`** и **`simulation1.log`**, а объединённый файл **переименуйте** в **`simulation.log`** — для `combineB2C_NOZIP.py` в **`with_groups`** должен остаться **один** файл с этим именем.

   ![Проводник: with_groups после переноса объединённого лога (перед очисткой и переименованием)](../images/MergeSimulation-merged-log-moved-to-with-groups.png)

21. **То же для `without_groups`:** в [`MergeSimulation.py`](../../tools/reporting/MergeSimulation.py) укажите **`input_folder`** на **`…/test1/test1/without_groups`** (с прямыми слэшами `/`, например `C:/Users/…/Отчет1/task2/test1/test1/without_groups`). Убедитесь, что в **`task2`** нет старого объединённого файла с тем же **`output_file`**, что и раньше, либо задайте другое имя **`output_file`**, чтобы не перезаписать результат по ошибке. Из **`task2`** выполните **`python MergeSimulation.py`** — скрипт по очереди склеит **`simulation.log`** и **`simulation2.log`**, сохранит объединённый файл в **`task2`** (в примере — **`общий_лог.log`**).
